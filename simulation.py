...
speeds = {'car':2.25, 'bus':1.8, 'truck':1.5, 'rickshaw':2, 'bike':2.5,"ambulance":3}  # average speeds of vehicles
...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        self.is_ambulance = vehicleClass == 'ambulance'
    ...
    def move(self):
        ...
        if self.is_ambulance and self.crossed:
            # Update the global ambulance lane variable
            print(f"{self.direction} crossed")
            # write_read("60")
...
def setTime():
    ...
    global ambulance_lane

    noOfCars, noOfBuses, noOfTrucks, noOfRickshaws, noOfBikes, noOfAmbulance = 0,0,0,0,0,0
    ...
    greenTime = math.ceil(((noOfCars*carTime) + (noOfRickshaws*rickshawTime) + (noOfBuses*busTime) + (noOfTrucks*truckTime)+ (noOfBikes*bikeTime) + (noOfAmbulance*ambulanceTime))/(noOfLanes+1))
...
class Main:
    ...
    background = pygame.image.load('images/base.png')
